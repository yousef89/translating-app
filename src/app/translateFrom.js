...
export default function TranslateFrom({
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch(
        `https://api.mymemory.translated.net/get?q=${segment}&langpair=${firstLanguage}|${secLanguage}`
      );
  ...
  return (
    //card container
    <div className="z-10 relative font-dm-sans font-semibold flex flex-col justify-center items-center bg-[#121826cc] rounded-[20px] bg-opacity-5 min-w-[360px] lg:w-[450px] xl:w-[650px]">
      <div className="flex justify-center items-center pl-5 pt-5 mr-auto">
        <div className="flex text-[15px] text-[#626874] gap-x-[5px] text-opacity-70 mr-auto">
          {languages.slice(0, 3).map((lang) => (
            <button
              key={lang.code}
              onClick={() => setFirstLanguage(lang.code)}
              className={
                firstLanguage === lang.code
                  ? "bg-[#4d5461] text-[#f4f6f8] rounded-[10px] py-2 px-2"
                  : "hover:bg-[#4d5461] hover:text-[#f4f6f8] rounded-[10px] py-2 px-2 duration-300"
              }
            >
              {lang.label}
            </button>
        ...
        {other && (
          <div className="z-10 absolute top-[16%] right-[2%] lg:right-[7%] xl:right-[35%] text-[11px] bg-gray-800 text-[#f4f6f8] p-2 rounded-md shadow-lg opacity-100 transition-opacity duration-1000">
            {languagesDrop.slice(0, 3).map((lang) => (
              <button
                key={lang.code}
                onClick={() => setFirstLanguage(lang.code)}
                className={
      ...
      <hr className="border-t border-[#384150] w-[85%] my-6 opacity-50" />

      <textarea
        className="w-[80%] text-[#F1F2F3] border-none outline-none text-opacity-80 bg-[#121826] pb-[124px] mx-7 mr-auto rounded-md bg-opacity-0 resize-none"
        maxLength={500}
        value={segment}
        onChange={segHandler}
        rows={4}
      ...
      <div className="flex items-center justify-center px-5 py-5 w-[100%]">
        <div className="flex gap-x-2 mr-auto">
          ...
          <div
            className="flex items-center border-[2px] border-gray-600 border-opacity-40 p-[4px] rounded-[6px] hover:border-[#838383] hover:bg-[#838383] active:bg-[#727272] duration-500 cursor-pointer "
        ...
        <p className="text-[#626874] text-opacity-70 text-[13px] ml-auto pr-6">
          {segment.length}/500
        </p>
      </div>
    </div>
